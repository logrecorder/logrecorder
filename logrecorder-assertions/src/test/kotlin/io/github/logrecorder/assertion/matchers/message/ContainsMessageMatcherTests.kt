...
internal class ContainsMessageMatcherTests {
    ...
    fun `message containing all supplied parts in any order matches`() {
        assertThat(matcherFor("XUR", "Foo").matches("Foo bar XUR")).isTrue()
    }
    ...
    fun `has an expressive toString value`() {
        assertThat(matcherFor("foo", "bar").toString()).isEqualTo("""contains ["foo", "bar"]""")
    }

    fun matcherFor(vararg parts: String) = ContainsMessageMatcher(parts.toList())
